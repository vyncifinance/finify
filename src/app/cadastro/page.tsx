...
export default function CadastroPage() {
  ...
  const inputStyle = { backgroundColor: '#fff', borderColor: '#E5E7EB', color: '#111827', height: '52px' }
    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-16" style={{ backgroundColor: '#F8F9FA' }}>
        <div className="w-full max-w-md">
          ...
          <h2 className="text-3xl font-bold mb-2" style={{ color: '#111827', letterSpacing: '-0.5px' }}>Criar conta</h2>
          <p className="text-sm mb-6" style={{ color: '#6B7280' }}>Preencha seus dados para começar</p>
          ...
          <div className="flex rounded-2xl p-1 mb-6" style={{ backgroundColor: '#E5E7EB' }}>
            {([['criar', 'Criar família'], ['entrar', 'Entrar com código']] as const).map(([val, label]) => (
              <button key={val} type="button" onClick={() => { setModo(val); setErro('') }}
                ...
                style={{ backgroundColor: modo === val ? '#fff' : 'transparent', color: modo === val ? '#0E3B2E' : '#6B7280', boxShadow: modo === val ? '0 1px 4px rgba(0,0,0,0.1)' : 'none', border: 'none', cursor: 'pointer' }}>
                {label}
              </button>
            ))}
          </div>
          ...
          <form onSubmit={handleCadastro} className="flex flex-col gap-4">
            ...
            <div>
              ...
              <input type="text" value={nome} onChange={e => setNome(e.target.value)}
                ...
                className="w-full px-4 rounded-2xl border text-sm outline-none transition-all" style={inputStyle}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowSenha(!showSenha)}
                  ...
                  {showSenha ? <EyeOff size={18} strokeWidth={1.75} /> : <Eye size={18} strokeWidth={1.75} />}
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowConfirmar(!showConfirmar)}
                  className="absolute right-4 top-1/2 -translate-y-1/2"
                  style={{ color: '#9CA3AF', background: 'none', border: 'none', cursor: 'pointer' }}>
                  {showConfirmar ? <EyeOff size={18} strokeWidth={1.75} /> : <Eye size={18} strokeWidth={1.75} />}
                </button>
              </div>
            </div>
            ...
            {erro && (
              <div className="rounded-xl px-4 py-3 text-sm" style={{ backgroundColor: '#FEF2F2', color: '#DC2626' }}>{erro}</div>
            )}
