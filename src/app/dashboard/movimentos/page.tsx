...
export default function MovimentosPage() {
  ...
  async function ajustarSaldoInvestimento(fid: string, dataStr: string, delta: number) {
    ...
    // Sem registro nesse mês ainda — só cria um novo se for um aporte (delta positivo),
    // carregando o saldo do mês anterior mais próximo como base
    if (delta > 0) {
  ...
  async function handleSalvar() {
    ...
    if (editando) {
      ...
      if (!error) {
        if (eraInvestimento) await ajustarSaldoInvestimento(fid, editando.data, -Number(editando.valor))
        if (eInvestimentoNovo) await ajustarSaldoInvestimento(fid, dataLanc, valorNum)
      }
      setSalvando(false)
      ...
      if (!error && ehCategoriaInvestimento(categoria)) {
        for (const item of inserts) await ajustarSaldoInvestimento(fid, item.data, item.valor)
      }
      setSalvando(false)
      if (!error) { setModalOpen(false); await carregarLancamentos(fid) }
    } else {
      ...
      if (!error && tipo === 'despesa' && ehCategoriaInvestimento(categoria)) {
        await ajustarSaldoInvestimento(fid, dataLanc, valorNum)
      }
      setSalvando(false)
      if (!error) { setModalOpen(false); await carregarLancamentos(fid) }
    }
  ...
  async function handleDeletar() {
    ...
    if (!error && editando.tipo === 'despesa' && ehCategoriaInvestimento(editando.categoria)) {
      await ajustarSaldoInvestimento(familiaIdRef.current, editando.data, -Number(editando.valor))
    }
    setDeletando(false)
    if (!error) {
